...
class App extends React.Component {
  ...
  setWeatherData = async () => {
    ...
    let requestWeatherApi = await Axios.get(
      `${APPHOST}/weather?q=${this.state.cityNameQuery}&lon=${this.state.lon}&lat=${this.state.lat}`
    );
  ...
  setMovieData = async () => {
    let APPHOST = process.env.REACT_APP_SERVER;
    let requestMovieApi = await Axios.get(
      `${APPHOST}/movies?q=${this.state.cityNameQuery}`
    );
